...
class ItemAutoOrderStep(object):
    ...
    def recurring_merchant_item_id(self, recurring_merchant_item_id):
        ...
        if not recurring_merchant_item_id:
            raise ValueError("Invalid value for `recurring_merchant_item_id`, must not be `None`")
        if len(recurring_merchant_item_id) > 20:
            raise ValueError("Invalid value for `recurring_merchant_item_id`, length must be less than `20`")
