...
class CartBilling(object):
    ...
    def email_confirm(self, email_confirm):
        ...
        if not email_confirm:
            raise ValueError("Invalid value for `email_confirm`, must not be `None`")
        if len(email_confirm) > 100:
            raise ValueError("Invalid value for `email_confirm`, length must be less than `100`")
