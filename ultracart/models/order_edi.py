...
class OrderEdi(object):
    ...
    def edi_internal_vendor_number(self, edi_internal_vendor_number):
        ...
        if not edi_internal_vendor_number:
            raise ValueError("Invalid value for `edi_internal_vendor_number`, must not be `None`")
        if len(edi_internal_vendor_number) > 50:
            raise ValueError("Invalid value for `edi_internal_vendor_number`, length must be less than `50`")
