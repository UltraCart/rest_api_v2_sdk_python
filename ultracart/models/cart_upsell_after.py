...
class CartUpsellAfter(object):
    ...
    def upsell_path_code(self, upsell_path_code):
        ...
        if not upsell_path_code:
            raise ValueError("Invalid value for `upsell_path_code`, must not be `None`")
        if len(upsell_path_code) > 5:
            raise ValueError("Invalid value for `upsell_path_code`, length must be less than `5`")
